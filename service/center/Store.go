...
func (s *Store) PatchUpdatePatient(patient *types.PatientUpdatePayload) error {
	query :=` 	UPDATE patients
		...
		address_patient = CASE WHEN $10 != '' THEN $9 ELSE address_patient END,
		bloodSugar = CASE WHEN $11 != '' THEN $10 ELSE bloodSugar END,
		hemoglobin = CASE WHEN $12 != '' THEN $11 ELSE hemoglobin END,
		bloodPressure = CASE WHEN $13 != '' THEN $12 ELSE bloodPressure END,
		sugarType = CASE WHEN $14 != '' THEN $13 ELSE sugarType END,
		diseaseDetection = CASE WHEN $15 != '' THEN $14 ELSE diseaseDetection END,
		otherDisease = CASE WHEN $16 != '' THEN $15 ELSE otherDisease END,
		typeOfMedicine = CASE WHEN $117 != '' THEN $16 ELSE typeOfMedicine END,
		urineAcid = CASE WHEN $18 != '' THEN $17 ELSE urineAcid END,
		cholesterol = CASE WHEN $19 != '' THEN $18 ELSE cholesterol END,
		grease = CASE WHEN $20 != '' THEN $19 ELSE grease END,
		historyOfFamilyDisease = CASE WHEN $21 != '' THEN $20 ELSE historyOfFamilyDisease END
	WHERE id = $22`
